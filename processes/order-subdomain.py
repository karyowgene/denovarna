#!/usr/bin/env python3
"""
order_var_domains_bestfirst.py
...
Usage:
  # Read from file:
  python3 order_var_domains_bestfirst.py domtbl_tabbed.txt [--debug]

  # Read from stdin (piped from grep or other command):
  grep pattern file.txt | python3 order_var_domains_bestfirst.py [--debug]
  cat domtbl_tabbed.txt | python3 order_var_domains_bestfirst.py [--debug]
"""
...
def main():
    p = argparse.ArgumentParser(
        ...
        epilog="If DOMTBL is omitted, reads from stdin (useful for piped input: grep ... | order-subdomain.py)",
    )
    p.add_argument(
        "domtbl",
        nargs="?",
